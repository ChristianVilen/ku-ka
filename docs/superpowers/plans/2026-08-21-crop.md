...
- [ ] **Step 1 — Cycle 1 (pixel size): add the failing test**
...
- [ ] **Step 2: Run red** — `-only-testing:KuKaTests/DrawingViewTests`. Expected: compile error, no `croppedTo` argument.
...
- [ ] **Step 4: Run green** — 2 tests pass.

- [ ] **Step 5 — Cycle 2 (orientation): add the failing test**
...
- [ ] **Step 6: Run** — with the mapping from Step 3 this should already pass (it is the guard against a future flip bug). If it fails on the colour read rather than on the mapping (byte order of the BGRA composite), draw `cropped` into a fresh `premultipliedLast` RGBA `CGContext` and read the first pixel's bytes instead of `colorAt`. Expected: 3 tests pass.

- [ ] **Step 7: Report** — 3/3 `DrawingViewTests` pass.
