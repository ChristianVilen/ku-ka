...
- [ ] **Step 1: Register both files in the project** (build note). Create both files empty, or with just the imports, so the project stays buildable.

- [ ] **Step 2 — Cycle 1 (draw): write the failing tests**
...
- [ ] **Step 3: Run red** — `-only-testing:KuKaTests/CropBoxTests`. Expected: compile error, `cannot find 'CropBox' in scope`.

- [ ] **Step 4 — Cycle 1: implement draw**
...
- [ ] **Step 5: Run green** — 2 tests pass.

- [ ] **Step 6 — Cycle 2 (move): add the failing tests**
...
- [ ] **Step 7: Run red** (the drag from inside draws a new box instead of moving). Then add: `action(at:)` returning `.move` when `rect.contains(point)`, the `.move` case of `beginDrag`, the `.move` case of `drag(to:)`. Run green — 4 pass.

- [ ] **Step 8 — Cycle 3 (handles): add the failing tests**
...
- [ ] **Step 9: Run red.** Then add: `handle(at:of:)`, the `.resize` branch in `action(at:)`, the `.resize` cases of `beginDrag` and `drag(to:)`. Run green — 8 pass.

- [ ] **Step 10 — Cycle 4 (clearing): add the failing tests**
...
- [ ] **Step 11: Run red.** Then add the size check in `endDrag`. Run green — 10 pass. The file now matches the target code above.

- [ ] **Step 12: Report** — 10/10 `CropBoxTests` pass; the build has no new warnings.
