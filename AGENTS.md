...
| `ClipboardItem` | Pure value type: kind is `.text(plain, rtf: Data?, html: Data?)` or `.image(png: Data, pixelSize)`, plus a SHA-256 content hash (via CryptoKit) that stays in step with `ImageStore`'s own hashing, copy date, `hasRichFlavors`, `byteCost`, and a one-line `previewLabel` built once up front |
...
- **Frameworks**: AppKit, CoreGraphics, ScreenCaptureKit, ServiceManagement, ApplicationServices (Accessibility API for window tiling), CryptoKit (SHA-256 content hashing for clipboard history)
